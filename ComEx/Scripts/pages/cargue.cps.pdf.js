...
function btnProcesar_Click(e) {
    ...
    if (success) {
        ...
        $.ajax({
            url: ResolveUrl("~/CpPdf/ProcesarArchivos"),
            type: "POST",
            contentType: false,
            cache: false,
            dataType: "json",
...
function TablaDatos() {
    tablaPrincipal = tablaPrincipal.DataTable({
        ...
        ajax: {
            type: "POST",
            url: ResolveUrl("~/CpPdf/GetTempCPs"),
            datatype: "json",
            complete: function () {                
...
function subirCPs() {
    ...
    $.ajax({
        ...
        beforeSend: function () {
            ...
            mensaje.html('<br /><div class="alert alert-info text-center"><i class="fa fa-cog fa-spin fa-3x fa-fw margin-bottom"></i> Cargando CPs, por favor espere.</div>').fadeIn(100);
        },
        success: function (resultado) {
            if (resultado.success) {
